...
CREATE OR REPLACE PACKAGE BODY PKG_LEVELUP_APP AS
    ...
    EXCEPTION
        WHEN DUP_VAL_ON_INDEX THEN
            DBMS_OUTPUT.PUT_LINE('Time j� existe: ' || P_TEAM_NAME);
            RAISE;
        WHEN OTHERS THEN
    ...
    BEGIN
        -- Valida��o simples de formato de e-mail com REGEXP
    ...
    EXCEPTION
        WHEN E_INVALID_EMAIL THEN
            DBMS_OUTPUT.PUT_LINE('E-mail inv�lido: ' || P_EMAIL);
            RAISE;
        WHEN DUP_VAL_ON_INDEX THEN
            DBMS_OUTPUT.PUT_LINE('E-mail j� cadastrado: ' || P_EMAIL);
            RAISE;
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('Erro ao criar usu�rio: ' || SQLERRM);
            RAISE;
    END PR_CREATE_USER;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('Erro ao criar atribui��o de tarefa: ' || SQLERRM);
            RAISE;
    ...
    EXCEPTION
        WHEN DUP_VAL_ON_INDEX THEN
            DBMS_OUTPUT.PUT_LINE('Usu�rio j� concluiu essa tarefa uma vez.');
            RAISE;
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('Erro ao registrar conclus�o de tarefa: ' || SQLERRM);
            RAISE;
...
PROCEDURE PR_CREATE_REWARD_REDEMPTION(
    ...
    BEGIN
        ...
        IF P_POINTS_SPENT IS NOT NULL AND P_POINTS_SPENT <> V_COST THEN
            RAISE_APPLICATION_ERROR(
                -20002,
                'Valor de pontos inv�lido para essa recompensa. Deve ser igual ao point_cost.'
            );
        END IF;
    ...
    EXCEPTION
        WHEN E_USER_NOT_FOUND THEN
            DBMS_OUTPUT.PUT_LINE('fn_build_user_profile_json: usu�rio n�o encontrado: ' || P_USER_ID);
            PR_WRITE_AUDIT('JSON_EXPORT', 'INSERT',
                           'USER_ID=' || P_USER_ID || ' N�O_ENCONTRADO');
            RETURN '{"error": "usuario_nao_encontrado", "user_id": ' || P_USER_ID || '}';

        WHEN E_NO_DATA_FOR_JSON THEN
            DBMS_OUTPUT.PUT_LINE('fn_build_user_profile_json: usu�rio sem dados para JSON: ' || P_USER_ID);
            PR_WRITE_AUDIT('JSON_EXPORT', 'INSERT',
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            DBMS_OUTPUT.PUT_LINE(
                'fn_analyze_user_future_profile: usu�rio n�o encontrado: ' || P_USER_ID
            );
            RETURN '{"error": "usuario_nao_encontrado", "user_id": ' || P_USER_ID || '}';
